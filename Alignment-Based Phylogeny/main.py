...
def solve_alph(tree_str, fasta):
    ...
    internal_seqs = {node["name"]: [] for node in post_order if node["children"]}
    ...
    for col in range(L):
            ...
            if not node["children"]:
                ...
                for s in range(5):
                    ...
                    for child in node["children"]:
                        ...
                        min_child_cost = min(dp[child_idx][sc] + cost(states[s], states[sc]) for sc in range(5))
        ...
        best_root_states = [s for s in range(5) if dp[root_idx][s] == min_root_cost]
...
def main():
    ...
    print(f"Parsed Newick tree of length {len(tree_str)}")
    print(f"Parsed {len(fasta)} leaf sequences.")
    
    cost, internal_seqs = solve_alph(tree_str, fasta)
    print(f"Minimum Hamming distance dH(T): {cost}")
    
    with open(output_path, 'w') as f:
        f.write(f"{cost}\n")
        # Write internal node sequences in FASTA format
        # We can write them in alphabetical order of their names to be clean
