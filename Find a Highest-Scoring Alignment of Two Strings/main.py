...
def read_input():
    ...
    input_file = os.path.join(script_dir, "rosalind_ba5e.txt")
...
# BLOSUM62 matrisi
# BLOSUM62 scoring matrix mapping
BLOSUM62 = {
...
# Needleman-Wunsch qlobal hizalama alqoritmi
# Implement global alignment using Needleman-Wunsch with constant gap penalty sigma = 5
def global_alignment(s1, s2):
    sigma = 5
    n, m = len(s1), len(s2)
    dp = [ [0] * (m + 1) for _ in range(n + 1) ]
    
    # Sərhəd şərtlərinin doldurulması
    # Initialize boundary conditions with gap penalties
    ...
    # Hizalanmanı bərpa edirik
    # Backtrack to reconstruct aligned sequences
    align1, align2 = [], []
    i, j = n, m
    while i > 0 or j > 0:
        if i > 0 and j > 0 and dp[i][j] == dp[i-1][j-1] + BLOSUM62[s1[i-1]][s2[j-1]]:
    ...
    return dp[n][m], "".join(align1), "".join(align2)

def main():
    s1, s2 = read_input()
    if not s1:
        return
    score, align1, align2 = global_alignment(s1, s2)
