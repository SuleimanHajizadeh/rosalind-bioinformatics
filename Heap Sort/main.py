...
# Heap Sort alqoritmini həyata keçirən funksiya.
# Function that implements the Heap Sort algorithm.
def heap_sort(arr):
    # Əvvəlcə massivi max-heap strukturuna salırıq.
    # First, build a max-heap from the array.
    build_max_heap(arr)
    size = len(arr)
    
    # Elementləri növbə ilə çıxarıb sıralayırıq.
    # One by one extract elements from the heap and sort.
    for i in range(size - 1, 0, -1):
        # Kök təpəni (ən böyük element) massivin sonuna keçiririk.
        # Move current root (largest element) to the end of the array.
        arr[0], arr[i] = arr[i], arr[0]
        # Kiçildilmiş heap-in kök təpəsi üçün heapify çağırırıq.
