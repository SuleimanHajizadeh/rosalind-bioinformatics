...
# Ağacın düyünlərini təmsil edən Node sinfi
# Node class representing nodes in the tree
...
def parse_tree(tokens):
    # Tokenlərdən ağac strukturunu qururuq
...
def collect_leaves(node, all_nodes):
    # Ağacdakı hər düyünün yarpaqlarını (leaves) toplayırıq
    # Collect all leaves for each node in the tree hierarchy
...
def main():
    ...
    splits = set()
    # Hər düyün üçün simvol bölünməsini (split) hesablayırıq
    # Generate splits representing partition of species for each internal node
    ...
    print(f"Generated character table with {len(sorted_splits)} splits.")
